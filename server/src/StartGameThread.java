...
public class StartGameThread extends Thread {
    ...
    public void run() {
        try {
            ...
            while (this.game.users.users.size() < 2)
                ; // PER TEST HO MESSO >2 MA DEVE ESSERE <2
            ...
            if (this.message.command.equals(CORRECT)) {
                ...
                if (this.game.discardedCards.getSizeDeck() > 0)
                    ...
                    this.game.discardedCard = this.game.discardedCards.getCardOnTop();
                else
                    // se non ci sono carte vuol dire che è appena
                    this.game.discardedCards.addCard(this.game.deck.getCard());

                Deck<Card> tmp = new Deck<>();
                // salvo nel deck tmp solo la carta appena scartata-> per non salvare tutto ilmazzo inutilmente
                tmp.addCard(this.game.discardedCards.deck.get(this.game.discardedCards.deck.size() - 1));
                String discarded_card_deck_serialized = tmp.serializeDeck();
